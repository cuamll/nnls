...
this will generate a set of 1000 matrices $A$ and solution vectors $`x_{ref}`$, does $`A x_{ref} = b`$ and then runs `solve` with $A$ and $b$ to get a solution $x$.
note that there might be multiple solutions of the equations it generates; for each iteration it prints $`\left|Ax -b\right|`$ and $`\left|x_{ref} - x\right|`$ so you can check the solution is reasonable and whether it's the same solution or not.
